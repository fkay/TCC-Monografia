...
\begin{align}
%\mathcal{H}(\sigma) = - \sum_{i = 1}^{n} \sum_{j = 1}^{m} \Bigl( \mathbbm{1}_{j \neq m}J((i,j),(i,j+1))\sigma_{i,j}\sigma_{i,j+1} + \mathbbm{1}_{i \neq n}J((i,j),(i+1),j)\sigma_{i,j}\sigma_{i+1,j} \Bigr)
%\mathcal{H}(\sigma) = - \sum\limits_{\langle s, s' \rangle } J(s,s')\sigma_s \sigma_{s'}  \mbox{  , com $s$ e $s'$ vizinhos}
